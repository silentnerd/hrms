...
class ApplyLeave extends React.Component {
  ...
  handleSubmit(event){
    ...
    axios.post(`http://localhost:5000/api/project`, JSON.stringify(projectData),{
      headers: {
...
console.log(value);
  ...
  componentDidMount(){
    axios.get('http://localhost:5000/api/nextprojectid').then(res =>
    {
      this.setState({nextid: res.data});
    });
  }
  ...
  return (
    <React.Fragment>
      ...
      <Col span={12}>
            ...
            layout='vertical'
          >
          <Input placeholder="Project ID" value={this.state.nextid} disabled />
          </Form.Item>
